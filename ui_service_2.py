...
class UIService2:
    def __init__(self, redis_url="redis://localhost", user_id="bob"):
    ...
    async def register_session(self):
        ...
        print(f"[UI2] Registered user '{self.user_id}' on service '{self.service_id}'")
                ...
                for stream_name, stream_messages in messages:
                    for message_id, message_data in stream_messages:
                        ...
                        if origin == self.service_id:
                            ...
                            print(f"[{datetime.now().strftime('%H:%M:%S')}] [UI2] Response from {origin}: '{word}' has length {length}")
